...
#### 1. 综合
 ...
 - 只使用双引号而非单引号。
 - 所有的属性名都必须在双引号内。
 - 字符类型属性的属性值必须用双引号，其他类型的（如布尔或数字）则不应该用。
 - 不要为了方便而把数据任意分组，只有在语义上有意义时候才使用它。
 
#### 2. 属性名准则
 - 属性名应当是有意义的，为驼峰形式，首字母必须是字母、下划线或者美元符号，随后的其他字符可以是字母、数字、下划线或美元符号。应当避免使用`Javascript`中的保留字。
 - 数组类型应当有复数的属性名，其他属性名都应该是单数的。
 
#### 3. 属性值准则
 - 属性值必须为以下几种类型之一：`boolean`、`number`、`string`、`object`、`array`、`null`。
 - 如果一个属性是可选的，或者包含空值，考虑把它从JSON里去掉。
 - 枚举值应当以字符串的形式呈现。
 - 日期应当使用RFC3339建议的格式。
   ```javascript
   {
